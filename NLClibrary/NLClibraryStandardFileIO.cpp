			...
			for(unordered_map<NLCgenericEntityClass*, NLCgenericEntityClass*>::iterator iter2 = NLCgenericEntityFromConditionList->begin(); iter2 != NLCgenericEntityFromConditionList->end(); iter2++)
			{
				...
				if(fromConditionObject->name == NLC_USE_LIBRARY_FILEIO_FUNCTION_ACTIONOBJECT_FILE_NAME)
				{
					...
					if(fileType == NLC_USE_LIBRARY_FILEIO_FILETYPE_XML)
					{
						...
						if(!readXMLfileToNLCpropertyListsEntry(firstTagInXMLFile, &readObjectClassList))
						{
							result = false;
						}
					}
	...
	if(currentTagInList->name == vectorListAllTagName)
	{
		...
		while(currentTagInVectorListAll->nextTag != NULL)
		{
			if(currentTagInVectorListAll->name == vectorListTagName)
			{
				...
				string vectorListName = currentTagInVectorListAll->name;	//extract the name of the vectorList from the first object in the list										
	#endif
	...
	if(currentTagInList->name == mapListAllTagName)
	{
		...
		while(currentTagInMapListAll->nextTag != NULL)
		{
			if(currentTagInMapListAll->name == mapListTagName)
			{
				...
				string mapListName1 = currentTagInVectorListAll->name;	//extract the name of the vectorList from the first object in the list
				string mapListName2 = "";
				if(currentTagInVectorListAll->firstAttribute->name == NLC_USE_LIBRARY_FILEIO_XML_ATTRIBUTENAME_CONDITION)
				{
					mapListName2 = currentTagInVectorListAll->firstAttribute->value;	//extract the name of the vectorList from the first object in the list (its attribute)
				}
				else
				{
