...
void writeXMLfileObject(XMLparserTag * firstTagInList, NLCgenericEntityClass* writeObject)
{
	...
	#endif
	for(unordered_map<pair<string, string>, unordered_map<NLCgenericEntityClass*, NLCgenericEntityClass*>  *> ::iterator iter2 = writeObject->conditionListAll.begin(); iter2 != writeObject->conditionListAll.end(); iter2++)
	{
	...
	#endif
	for(unordered_map<string, vector<NLCgenericEntityClass*>  *> ::iterator iter2 = writeObject->actionIncomingListAll.begin(); iter2 != writeObject->actionIncomingListAll.end(); iter2++)
	{
		string propertyListKey = iter2->first;
		vector<NLCgenericEntityClass*> * propertyList = iter2->second;
	...
	#endif
	for(unordered_map<string, vector<NLCgenericEntityClass*>  *> ::iterator iter2 = writeObject->actionSubjectListAll.begin(); iter2 != writeObject->actionSubjectListAll.end(); iter2++)
	{
		string propertyListKey = iter2->first;
		vector<NLCgenericEntityClass*> * propertyList = iter2->second;
	...
	#endif
	for(unordered_map<string, vector<NLCgenericEntityClass*>  *> ::iterator iter2 = writeObject->actionObjectListAll.begin(); iter2 != writeObject->actionObjectListAll.end(); iter2++)
	{
		string propertyListKey = iter2->first;
		vector<NLCgenericEntityClass*> * propertyList = iter2->second;
...
void writeNLCflatFilePropertyLists(string fileName, vector<NLCgenericEntityClass*>* writeObjectClassList, char delimiter)
{
	...
	for(vector<NLCgenericEntityClass*> ::iterator iter1 = writeObjectClassList->begin(); iter1 < writeObjectClassList->end(); iter1++) 
	{
		...
		if(firstLineWriteHeader)
		{
			string fileHeader = "";
			bool firstRow = true;
			for(unordered_map<string, vector<NLCgenericEntityClass*> *> ::iterator iter2 = parentObject->propertyListAll.begin(); iter2 != parentObject->propertyListAll.end(); iter2++) 
			{
				if(!firstRow)
				{
