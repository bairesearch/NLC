...
{
	#ifdef NLC_USE_LIBRARY_FILEIO_XML_WRITE_LIST_TAGS_ENTRY
...
{
	writeXMLfileMapList(firstTagInList, writeObjectClassMapList, NLC_USE_LIBRARY_FILEIO_XML_TAGNAME_CONDITION_LIST);
}
...
XMLparserTag* writeXMLfileObject(XMLparserTag* firstTagInList, NLCgenericEntityClass* writeObject)
{
	...
	#ifdef NLC_USE_LIBRARY_FILEIO_XML_WRITE_PROPERTY_LISTS
	#ifdef NLC_USE_LIBRARY_FILEIO_XML_WRITE_LIST_TAGS
	currentTagInListL2 = createListTagWithLowerLevel(currentTagInList, NLC_USE_LIBRARY_FILEIO_XML_TAGNAME_PROPERTY_LIST_ALL);
	#endif
	...
	#ifdef NLC_USE_LIBRARY_FILEIO_XML_WRITE_CONDITION_LISTS
	#ifdef NLC_USE_LIBRARY_FILEIO_XML_WRITE_LIST_TAGS
	currentTagInListL2 = createListTagWithLowerLevel(currentTagInList, NLC_USE_LIBRARY_FILEIO_XML_TAGNAME_CONDITION_LIST_ALL);
	#endif
	...
	#ifdef NLC_USE_LIBRARY_FILEIO_XML_WRITE_ACTION_LISTS
	#ifdef NLC_USE_LIBRARY_FILEIO_XML_WRITE_LIST_TAGS
	currentTagInListL2 = createListTagWithLowerLevel(currentTagInList, NLC_USE_LIBRARY_FILEIO_XML_TAGNAME_ACTION_LIST_ALL);
	#endif
	for(unordered_map<string, vector<NLCgenericEntityClass*>*>::iterator iter2 = writeObject->propertyLists.begin(); iter2 != writeObject->propertyLists.end(); iter2++)
	{
		string propertyListKey = iter2->first;
	...
	#ifdef NLC_USE_LIBRARY_FILEIO_XML_WRITE_LIST_TAGS
	currentTagInListL2 = createListTagWithLowerLevel(currentTagInList, NLC_USE_LIBRARY_FILEIO_XML_TAGNAME_ACTIONINCOMING_LIST_ALL);
	#endif
	...
	#ifdef NLC_USE_LIBRARY_FILEIO_XML_WRITE_LIST_TAGS
	currentTagInListL2 = createListTagWithLowerLevel(currentTagInList, NLC_USE_LIBRARY_FILEIO_XML_TAGNAME_ACTIONSUBJECT_LIST_ALL);
	#endif
	...
	#ifdef NLC_USE_LIBRARY_FILEIO_XML_WRITE_LIST_TAGS
	currentTagInListL2 = createListTagWithLowerLevel(currentTagInList, NLC_USE_LIBRARY_FILEIO_XML_TAGNAME_ACTIONOBJECT_LIST_ALL);
	#endif
...
void readFunction(vector<NLCgenericEntityClass*>& readClassList, vector<NLCgenericEntityClass*>& readObjectClassList)	
{
	...
	for(vector<NLCgenericEntityClass*>::iterator iter1 = readClassList.begin(); iter1 < readClassList.end(); iter1++) 
	{
		...
		//added 1q14i
		unordered_map<NLCgenericEntityClass*, NLCgenericEntityClass*>* NLCgenericEntityToConditionList = getConditionListByConditionName(readAction, NLC_USE_LIBRARY_FILEIO_FUNCTION_ACTION_CONDITION_TO_NAME); 
			...
			for(unordered_map<NLCgenericEntityClass*, NLCgenericEntityClass*>::iterator iter2 = NLCgenericEntityFromConditionList->begin(); iter2 != NLCgenericEntityFromConditionList->end(); iter2++)
			{
				...
				if(fromConditionObject->name == NLC_USE_LIBRARY_FILEIO_FUNCTION_ACTIONOBJECT_FILE_NAME)
				{
					...
					else
					{
						...
						{
							fileName = string(NLC_USE_LIBRARY_FILEIO_FUNCTION_DEFAULT_FILE_NAME_PREPEND) + (readObjectClassList.back())->name + string(NLC_USE_LIBRARY_FILEIO_FUNCTION_DEFAULT_FILE_NAME_APPEND_XML);	//eg read the People from the file (NLCfileIOPeople.xml)
						}
						else
						{
...
string getTopLevelPropertyListNameFromXMLreadFileContents(XMLparserTag* firstTagInXMLFile)
{
	...
	if(firstTagInXMLFile->firstAttribute != NULL)
	{
		...
		{
			cout << "getTopLevelPropertyListNameFromXMLreadFileContents{} error: firstTagInXMLFile->firstAttribute->name != NLC_USE_LIBRARY_FILEIO_XML_ATTRIBUTENAME_LISTNAME" << endl;
		}
	}
	else
	{
...
{
	return readXMLfileMapList(firstTagInList, readObjectClassMapList, NLC_USE_LIBRARY_FILEIO_XML_TAGNAME_CONDITION_LIST);
}
	...
	if(currentTagInList->name == mapListAllTagName)
	{
		...
		while(currentTagInMapListAll->nextTag != NULL)
		{
			if(currentTagInMapListAll->name == mapListTagName)
			{
				...
				{
					cout << "readFileObjectMapListAll error(): " << mapListTagName << ": currentTagInMapListAll->firstAttribute->name != NLC_USE_LIBRARY_FILEIO_XML_ATTRIBUTENAME_MAPLISTNAME1" << endl;
					result = false;
				}
				string mapListName2 = "";
				if(currentTagInMapListAll->firstAttribute->nextAttribute != NULL)
				{
					...
					{
						cout << "readFileObjectMapListAll error(): " << mapListTagName << ": currentTagInMapListAll->firstAttribute->nextAttribute->name != NLC_USE_LIBRARY_FILEIO_XML_ATTRIBUTENAME_MAPLISTNAME2" << endl;
						result = false;
					}
				}	
				else
	...
	for(vector<NLCgenericEntityClass*>::iterator iter1 = writeObjectClassList->begin(); iter1 < writeObjectClassList->end(); iter1++) 
	{
		...
		for(unordered_map<string, vector<NLCgenericEntityClass*>*>::iterator iter2 = parentObject->propertyLists.begin(); iter2 != parentObject->propertyLists.end(); iter2++) 
		{
			...
			if(!(childObjectList->empty()))
			{
				...
				else if(childObject->mathObjectType == NLC_USE_MATH_OBJECTS_VARIABLE_TYPE_NUMERICAL)
				{
				...
				else if(childObject->mathObjectType == NLC_USE_MATH_OBJECTS_VARIABLE_TYPE_STRING)
				{
				...
				else if(childObject->mathObjectType == NLC_USE_MATH_OBJECTS_VARIABLE_TYPE_BOOLEAN)
				{
					...
					{	
						...
						for(int i=0; i<numberOfVariables; i++)
						{
							...
							else
							{
								...
								{
									readObject->mathObjectNumericalValue = convertStringToDouble(variableValue);
									readObject->mathObjectType = NLC_USE_MATH_OBJECTS_VARIABLE_TYPE_NUMERICAL;
								}
								#ifdef NLC_USE_MATH_OBJECTS_ADVANCED
								#ifdef NLC_USE_MATH_OBJECTS_STRING
								else if((variableValue.length() >= 3) && (variableValue[0] == NLC_USE_MATH_OBJECTS_STRING_DELIMITER_CHAR) && (variableValue[variableValue.length()-1] == NLC_USE_MATH_OBJECTS_STRING_DELIMITER_CHAR))
								{
									readObject->mathObjectStringValue = variableValue.substr(1, variableValue.length()-2);
									readObject->mathObjectType = NLC_USE_MATH_OBJECTS_VARIABLE_TYPE_STRING;
									#ifdef NLC_DEBUG_LIBRARY
								...
								else if(variableValue == NLC_USE_LIBRARY_FILEIO_FLAT_NULL_PROPERTY_VALUE)
								{
									saveVariable = false;
								}
								else
