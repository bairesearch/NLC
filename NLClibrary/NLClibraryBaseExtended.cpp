...
void addEnumProperty(E1* E1Instance, E2* E2Instance, vector<NLCgenericEntityClass*>* propertyList)
{
	...
	if(enumTypeTablePropertiesIter != enumTypeTableProperties.end())
	{
		...
		E1Instance->enumPropertyLists[E2Instance->enumType].insert(pair<string, vector<E2*>*>(E2Instance->name, propertyList));
		//E1Instance->enumPropertyLists.insert(pair<string, unordered_map<string, vector<E2*>*>>(E2Instance->enumType, pair<string, vector<E2*>*>(new pair<string, vector<E2*>*>(E2Instance->name, propertyList))));
	}	
}
...
void clearExistingEnumProperty(E1* E1Instance, E2* E2Instance)
{
	...
	{
		unordered_map<string, unordered_map<string, vector<NLCgenericEntityClass*>*>>::iterator enumPropertyListsIter = E1Instance->enumPropertyLists.find(enumType);
		if(enumPropertyListsIter != E1Instance->enumPropertyLists.end())
		{//enumTypePropertyList found
			...
			for(unordered_map<string, vector<NLCgenericEntityClass*>*>::iterator enumTypePropertyListsIter = enumTypePropertyLists->begin(); enumTypePropertyListsIter != enumTypePropertyLists->end(); enumTypePropertyListsIter++) 
			{
				...
				for(vector<NLCgenericEntityClass*>::iterator iter1 = enumTypePropertyList->begin(); iter1 < enumTypePropertyList->end(); iter1++) 
				{
					...
					for(typename vector<E1*>::iterator iter2 = enumTypeProperty->propertyIncomingList.begin(); iter2 < enumTypeProperty->propertyIncomingList.end(); iter2++) 
					{
						E1* parentEntity = *iter2;
						if(parentEntity != E1Instance)
						{
							cout << "clearExistingEnumPropertyGeneric{} warning: enum property (quality) detected which is a child (property) of more than one entity: entity = " << E1Instance->name << ", property = " << E2Instance->name << endl;
							vector<NLCgenericEntityClass*>* parentEntityPropertyList = getPropertyListByName(parentEntity, enumTypeProperty->name);
							removeItemFromVectorList<NLCgenericEntityClass>(parentEntityPropertyList, enumTypeProperty);
						}
					}
...
void addEnumCondition(E1* E1Instance, E2* E2Instance, E3* E3Instance, unordered_map<NLCgenericEntityClass*, NLCgenericEntityClass*>* conditionList)
{
	...
	if(enumTypeTableConditionsIter != enumTypeTableConditions.end())
	{
		...
		E1Instance->enumConditionLists[E2Instance->enumType].insert(pair<pair<string, string>*, unordered_map<E2*, E3*>*>(new pair<string, string>(E2Instance->name, E3Instance->name), conditionList));
		//E1Instance->enumConditionLists.insert(pair<string, pair<pair<string, string>*, unordered_map<E2*, E3*>*>>(E2Instance->enumType, pair<pair<string, string>*, unordered_map<E2*, E3*>*>(new pair<string, string>(E2Instance->name, E3Instance->name), conditionList)));
	}
}
...
void clearExistingEnumCondition(E1* E1Instance, string E2className, E3* E3Instance)
{
	...
	if(enumType != NLC_ENUM_LIST_TYPE_DEFAULT_NON_ENUM)
	{
		unordered_map<string, unordered_map<pair<string, string>*, unordered_map<NLCgenericEntityClass*, NLCgenericEntityClass*>*>>::iterator enumConditionListsIter = E1Instance->enumConditionLists.find(enumType);
		if(enumConditionListsIter != E1Instance->enumConditionLists.end())
		{//enumTypePropertyList found
...
bool removeItemFromUnorderedMap(unordered_map<E1*, E2*>* unorderedMap, string conditionName, E2* itemToRemove)
{
	...
	*/
	...
	for(typename unordered_map<E1*, E2*>::iterator iter1 = unorderedMap->begin(); iter1 != unorderedMap->end();) 
	{
