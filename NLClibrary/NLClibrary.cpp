...
void initiateEnumTypeTables(XMLparserTag* NLCfirstTagInXMLfile)
{
	...
	bool result = true;
	XMLparserTag* firstTagInRulesTag = parseTagDownALevel(NLCfirstTagInXMLfile, RULES_XML_TAG_rules, &result);
	if(result)
	{
			...
			while(currentEntityTag->nextTag != NULL)
			{
				if(currentEntityTag->name == RULES_XML_TAG_entity)
				{
					...
					if(currentEntityTag->firstAttribute->name == RULES_XML_ATTRIBUTE_name)
					{
						...
						{
							enumTypeTableCurrent = &enumTypeTableCondition;
						}
						else if(currentEntity == NLC_ENUM_LIST_ENTITY_NAME_ACTION)
						{
							enumTypeTableCurrent = &enumTypeTableAction;
						}
						else
						{
					...
					firstTagInEntityTag = parseTagDownALevel(currentEntityTag, RULES_XML_TAG_entity, &result);
					...
					while(currentTypeTag->nextTag != NULL)
					{
						...
						{	
							...
							firstTagInTypeTag = parseTagDownALevel(currentTypeTag, RULES_XML_TAG_type, &result);
