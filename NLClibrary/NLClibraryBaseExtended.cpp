...
#ifdef NLC_USE_MATH_OBJECTS_STRING_DYNAMIC_DETERMINE_MATH_OBJECT_VARIABLE_TYPE
...
{
	//boolean expression, eg "the red chicken is next to the house" (theredchickenisnexttothehouse) <- NB boolean expressions contain the auxiliary "is"
...
bool testMathObjectValue(NLCgenericEntityClass* targetObject, NLCgenericEntityClass* sourceObject, string testExpression)
{
	bool testResult = false;
	int mathObjectVariableType = NLC_USE_MATH_OBJECTS_VARIABLE_TYPE_UNKNOWN;
	bool mathObjectVariableFoundForBOth
	if(sourceObject->numericalValue != numeric_limits<double>::quiet_NaN())
	{
		if(targetObject->numericalValue != numeric_limits<double>::quiet_NaN())
		{
			mathObjectVariableType = NLC_USE_MATH_OBJECTS_VARIABLE_TYPE_NUMERICAL;
			testResult = testMathObjectValue(sourceObject->numericalValue, targetObject->numericalValue, testExpression);
		}
		else
		{
	...
	else if(sourceObject->stringValue != "")
	{
		...
		{
			mathObjectVariableType = NLC_USE_MATH_OBJECTS_VARIABLE_TYPE_STRING;
			testResult = executeStringExpression(sourceObject->stringValue, targetObject->stringValue, testExpression);
...
bool testMathObjectValue(NLCgenericEntityClass* targetObject, double sourceObject, string testExpression)
{
	return testMathObjectValue(targetObject->numericalValue, sourceObject, testExpression);
}
bool testMathObjectValue(double targetObject, NLCgenericEntityClass* sourceObject, string testExpression)
{
	return testMathObjectValue(targetObject, sourceObject->numericalValue, testExpression);
}
bool testMathObjectValue(double targetObject, double sourceObject, string testExpression)
{
	bool testResult = false;
	if(testExpression == NLC_PREPROCESSOR_MATH_OPERATOR_STRING_EQUALS_TEST)
	{
		testResult = (targetObject == sourceObject);
	}
	else if(NLC_PREPROCESSOR_MATH_OPERATOR_GREATER_THAN)
	{
		testResult = (targetObject > sourceObject);
	}
	else if(testExpression == NLC_PREPROCESSOR_MATH_OPERATOR_LESS_THAN)
	{
		testResult = (targetObject < sourceObject);
	}
	else if(testExpression == NLC_PREPROCESSOR_MATH_OPERATOR_GREATER_THAN_OR_EQUAL_TO)
	{
		testResult = (targetObject >= sourceObject);
	}
	else if(testExpression == NLC_PREPROCESSOR_MATH_OPERATOR_LESS_THAN_OR_EQUAL_TO)
	{
		testResult = (targetObject <= sourceObject);
	}
	else if(testExpression == NLC_PREPROCESSOR_MATH_OPERATOR_DOES_NOT_EQUAL)
	{
		testResult = (targetObject != sourceObject);
	}
	else
	{
		cout << "testMathObjectValue{} error: testing mathobject numerical value but testExpression is invalid; sourceObject->name = " << sourceObject->name << ", targetObject->name = " << targetObject->name << ", testExpression = " << testExpression << endl; 	
	}
	return testResult;					
}

//NB supported testExpression values: NLC_PREPROCESSOR_MATH_OPERATOR_EQUALS_TEST
bool testMathObjectValue(string targetObject, string sourceObject, string testExpression)
{
	bool testResult = false;
	if(testExpression == NLC_PREPROCESSOR_MATH_OPERATOR_EQUALS_TEST
	{
		testResult = (targetObject == sourceObject);
	}
	else
	{
		cout << "testMathObjectValue{} error: testing mathobject string value but testExpression is invalid; sourceObject->name = " << sourceObject->name << ", targetObject->name = " << targetObject->name << ", testExpression = " << testExpression << endl; 		
	}
	return testResult;
}
#endif
