				...
				if(foundLogicalConditionOperation)
				{
					...
					{//ignore networkIndexes
						...
						if(foundConditionSubject && foundConditionObject)
						{
							...
							{
								cout << "identifyAndTagAllLogicalConditionOperations{} error: NLC_SUPPORT_LOGICAL_CONDITION_OPERATIONS_BASED_ON_CONCEPTS only handles concepts. GIA_CREATE_CONCEPTS_FOR_ALL_SENTENCES_WITH_NETWORK_INDEXES must be enabled." << endl;
...
bool createFunctionDefinitionClassDefinition(vector<NLCclassDefinition*>* classDefinitionList, string NLCfunctionName, int functionDefinitionIndex, bool libraryFunctionDeclaration)
{
	...
	{	
		...
		if(libraryFunctionDeclaration)
		{
			...
			string actionClassName =  generateClassName(functionName);
