...
void generateCodeFindAliasAndAddToCategoryListNewFunction(const int progLang, string* code, const int level)
{
	...
	q++;
		...
		printLine(progLangOpenBlock[progLang], level+q, code);	//{
			q++;
			string tempVarDeclarationText = generateCodeEntityDefinitionText(templateName1, tempVariableName, progLang) + progLangEquals[progLang] + generateCodeIterReference(iterIndexString, progLang) + progLangEndLine[progLang];	//E1* genericEntity = *iter1;
...
void generateCodeCheckParentClassNameNewFunction(const int progLang, string* code, const int level)
{
	...
	q++;
		...
		printLine(progLangOpenBlock[progLang], level+q, code);
		q++;
			string tempVarDeclarationText = generateCodeEntityDefinitionText(genericEntityClassName, parentName, progLang) + progLangEquals[progLang] + generateCodeIterReference(iterIndexString, progLang) + progLangEndLine[progLang];	//NLCgenericEntityClass* parent = *iter1;
			printLine(tempVarDeclarationText, (level+q), code);
...
void generateCodeAddNewEntityToLocalList(const NLCitem* param1, const int progLang, string* code, const int level)
{
	...
	#else
	string codeBlockTextCreate = generateCodeNewEntity(param1, progLang);		//const param1className* param1 = new param1className();
...
void generateCodeVerifyDefiniteReferenceExistenceExecuteFunction(const NLCitem* param1, const NLCitem* param2, const int progLang, string* code, const int level)
{
	...
	string testEmptyText = generateGenericListName(param1->genericObjectName, genericListAppendName) + progLangObjectReferenceDelimiter2[progLang] + progLangHasEntity[progLang];		//param1CategoryList.empty()
